...
APP_REVISION = "20251004.02 Fixed" 
# ------------------------------------------
...
class MainUIBase:
    ...
    def _srm_to_hex(self, srm):
        ...
        # UPDATED: Allow 0 for Clear/White
        srm = max(0, min(40, int(srm)))
        
        # Standard SRM Color Map
        srm_colors = {
            # NEW: SRM 0 = White (for Tonic, Seltzer, Water)
            0: "#FFFFFF",
            1: "#F0EFB5", 2: "#F9E06C", 3: "#F4D23F", 4: "#F0BF1E",
            5: "#E9AF0C", 6: "#D89E0A", 7: "#C98D08", 8: "#BF7C07",
            9: "#B56D06", 10: "#A95E05", 11: "#9D5105", 12: "#924504",
            13: "#883B04", 14: "#7D3203", 15: "#742903", 16: "#6B2102",
            17: "#621A02", 18: "#5A1402", 19: "#520F01", 20: "#4B0B01",
            21: "#440801", 22: "#3E0501", 23: "#380401", 24: "#320300",
            25: "#2D0200", 26: "#280200", 27: "#240100", 28: "#200100",
            29: "#1C0100", 30: "#180100", 31: "#150000", 32: "#120000",
            33: "#0F0000", 34: "#0D0000", 35: "#0A0000", 36: "#080000",
            37: "#060000", 38: "#040000", 39: "#020000", 40: "#000000"
        }
        return srm_colors.get(srm, "#E9AF0C") # Default to SRM 5 (Pale/Gold) if missing
    ...
    def _define_progressbar_styles(self):
        if not self.progressbar_styles_defined:
            ...
            s.configure("neutral.Horizontal.TProgressbar", background='#C0C0C0', **common_opts) # Gray/Neutral
            ...
            # --- NEW: Initialize unique styles for each tap ---
            # This allows us to set a different color for Tap 1 vs Tap 2
            # These will be dynamically updated by _update_tap_progress_bar_colors
    ...
    def _update_tap_progress_bar_colors(self):
        """
        OPTIMIZATION: Updates the style configuration for all tap progress bars.
        This should ONLY be called when beverage assignments or definitions change,
        NOT inside the high-frequency sensor update loop.
        """
        try:
            ...
            for i in range(self.num_sensors):
                ...
                if i < len(assignments):
                    ...
                    # --- NEW: Check for Offline ID explicitly ---
                    if b_id == UNASSIGNED_BEVERAGE_ID:
                        # Use a neutral gray for offline taps
                        bar_color = '#C0C0C0' 
                    else:
                        ...
                        if beverage:
                            ...
                            # Default to 5 (Gold) if beverage exists but has no SRM
                ...
                # Update the unique style for this tap
                style_name = f"Tap{i}.Horizontal.TProgressbar"
    ...
    def _do_update_sensor_stability_display(self, sensor_index, status_text_from_logic):
        ...
        # --- OPTIMIZED LOGIC: No heavy lifting here ---
        # The styles (TapX...) are pre-calculated in _update_tap_progress_bar_colors
        if status_text_from_logic == "Data stable":
            
            # Use the pre-configured tap-specific style
            ...
            self.was_stable_before_pause[sensor_index] = True
             ...
             # All non-stable states use neutral gray
            ...
            # Special case for Paused (keep percentage but gray out)
            new_style = "neutral.Horizontal.TProgressbar" 
    ...
    def _populate_beverage_dropdowns(self):
        ...
        # Add mapping for the offline ID
        ...
        for i in range(self.num_sensors):
            ...
            if assigned_name:
                ...
                # Default logic: If assignment invalid/missing, select first item (Offline)
    ...
    def _handle_beverage_selection_change(self, sensor_idx):
        ...
        if selected_beverage_name == "Offline":
            ...
            print(f"UIManager: Tap {sensor_idx+1} assigned to Beverage: Offline")
            self._refresh_beverage_metadata()
            return
        # -------------------------------------
    ...
    def _refresh_beverage_metadata(self):
        # --- OPTIMIZATION: Update colors here when metadata refreshes ---
    ...
    def _handle_keg_selection_change(self, sensor_idx):
        ...
        # --- NEW: Intercept "Keg Kicked" Action ---
        if selected_keg_title.startswith("★"):
            # 1. Revert the dropdown to the currently assigned keg immediately
            # This ensures that if they hit 'No' or cancel, the UI is already back to normal state
            current_assignments_ids = self.settings_manager.get_sensor_keg_assignments()
            assigned_id = current_assignments_ids[sensor_idx]
            
            # Resolve ID to Title
            keg = self.settings_manager.get_keg_by_id(assigned_id)
            # Handle UNASSIGNED_KEG_ID special case returned by get_keg_by_id
            if keg and keg.get('id') == UNASSIGNED_KEG_ID:
                 previous_title = "Offline"
            elif keg:
                 previous_title = keg.get('title', 'Unknown')
            else:
                 previous_title = "Offline"
            
            # Reset the UI variable immediately
            self.sensor_keg_selection_vars[sensor_idx].set(previous_title)
            
            # 2. INTERMEDIARY CONFIRMATION DIALOG
            confirm = messagebox.askyesno(
                "Confirm Keg Kicked",
                f"Has the keg on Tap {sensor_idx+1} completely run dry?\n\n"
                "This function is for calibrating the tap using a known full keg volume "
                "against the pulses recorded until empty.\n\n"
                "Click YES only if the keg is empty.",
                parent=self.root
            )
            
            if not confirm:
                return # User cancelled or mis-clicked
            
            # 3. Open the Calibration Popup pre-populated
            # Tab Index 1 = Keg Calibration (0 is Pour Cal)
            if hasattr(self, '_open_flow_calibration_popup'):
                print(f"UIManager: 'Keg Kicked' confirmed for Tap {sensor_idx+1}. Launching calibration...")
