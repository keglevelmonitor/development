...
class PopupManagerMixin:    
    ...
    def _save_keg_definition(self, temp_vars, is_new, popup_window, parent_popup):
        try:
            ...
            final_pulses_to_save = 0 # Default to reset on new/change
            ...
            if existing_keg:
                ...
                # Check if the current remaining volume was manually set/changed
                ...
                # If starting volume changed (Reset Keg), we reset pulses to 0 (already default).
                # If starting volume did NOT change, we preserve the pulse count.
                if not starting_vol_changed:
                     ...
                     print(f"UIManager: Keg volume reset (weight change). Resetting pulse count to 0.")
            ...
            new_data = {
                ...
                "total_dispensed_pulses": final_pulses_to_save # Save tracked pulses
            }
    ...
    def _save_keg_calibration(self, popup):
        try:
            ...
            # 2. Set the Keg assignment to Offline (since the keg is kicked)
            self.settings_manager.save_sensor_keg_assignment(tap_idx, UNASSIGNED_KEG_ID)

            # 3. NEW: Set the Beverage (Tap) to Offline as well
            self.settings_manager.save_sensor_beverage_assignment(tap_idx, UNASSIGNED_BEVERAGE_ID)

            # 4. Force Logic Refresh (Recalculates with new K-Factor)
            if hasattr(self, 'sensor_logic') and self.sensor_logic:
                self.sensor_logic.force_recalculation()
            
            # 5. Force UI Refresh (Updates dropdowns to "Offline" and data display)
            # This calls the method inherited from MainUIBase
            ...
            messagebox.showinfo("Success", 
                                ...
                                "The Tap and Keg have been set to 'Offline' because the keg is empty.", 
                                parent=popup)
            popup.destroy()
            
        except Exception as e:
            messagebox.showerror("Error", f"Could not save calibration: {e}", parent=popup)
    ...
    def _open_uninstall_app_popup(self):
        ...
        popup.geometry("500x420") # Slightly adjusted height for spacing
        popup.transient(self.root)
        popup.grab_set()
        
        # Center popup
        self._center_popup(popup, 500, 420)
        ...
        # Warning Text (Restored to warning only)
        warning_text = (
            "This action will completely remove the KegLevel Monitor application and ALL associated data.\n\n"
            "• The application folder (~/keglevel) will be deleted.\n"
            "• The data folder (~/keglevel-data) will be deleted.\n"
            "• All settings, history, and beverage libraries will be lost.\n"
            "• The autostart entry will be removed.\n\n"
            "This action CANNOT be undone."
        )
        ttk.Label(main_frame, text=warning_text, wraplength=450, justify="left").pack(pady=(0, 15))
        ...
        # NEW LOCATION: Reset Hint Text
        reset_text = (
            "To reset all settings to their default values, click Cancel to close this "
            "box and select 'Reset to Defaults' from the settings menu."
        )
        # MODIFIED: Removed font argument so it matches the warning text size
        ttk.Label(main_frame, text=reset_text, wraplength=450, justify="left").pack(pady=(0, 10))
        ...
        uninstall_btn = ttk.Button(btn_frame, text="Uninstall & Delete", state="disabled",
                                   command=lambda: self._execute_uninstall_app(popup, confirm_var))
        uninstall_btn.pack(side="right", padx=5)
        
        ttk.Button(btn_frame, text="Cancel", command=popup.destroy).pack(side="right", padx=5)

        # Trace to enable button only when "YES" is typed
    ...
    def _execute_uninstall_app(self, popup_window, confirm_var):
        if confirm_var.get() != "YES":
            return
        ...
        try:
            ...
            # 2. Remove Autostart (if active)
            if IS_RASPBERRY_PI_MODE:
                try:
                    # Assuming manage_autostart_file is imported from main
                    manage_autostart_file('remove', app_root_dir)
                    print("Uninstall: Autostart entry removed.")
                except Exception as e:
                    print(f"Uninstall Warning: Could not remove autostart: {e}")

            # 3. Delete Data Directory (~/keglevel-data)
            if os.path.exists(data_dir):
                shutil.rmtree(data_dir)
                print(f"Uninstall: Deleted data directory: {data_dir}")

            # 4. Delete App Directory (~/keglevel)
            # Note: We are running code FROM this directory. On Linux/Pi this is allowed, 
            # but the process must exit immediately after.
            ...
            messagebox.showinfo("Uninstall Complete", 
                                "The application and all data have been successfully removed.\n\n"
                                "The program will now exit.", 
                                parent=popup_window)
            
            popup_window.destroy()
            self._on_closing_ui() # Graceful shutdown logic
