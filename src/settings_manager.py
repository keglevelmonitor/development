...
# FIX: Moved to module level so UI/other modules can import and reference it.
...
class SettingsManager:
    ...
    def _get_default_keg_definitions(self):
        ...
        for i in range(5):
            keg_def = {
                "id": str(uuid.uuid4()),
                "title": f"Keg {i+1:02}",
                # RENAMED: Empty Weight to Tare Weight
                "tare_weight_kg": 4.50,         
                # Starting Total Weight remains the same key, but is clearer in the UI
                "starting_total_weight_kg": 23.50,
                # NEW: Maximum Full Volume for graphic display/reference (5 US Gallons)
                "maximum_full_volume_liters": 18.93,
                # NEW/RENAMED: Store the derived volume separate from the dispensed volume
                "calculated_starting_volume_liters": 18.90, 
                "current_dispensed_liters": 0.0,
                # NEW: Track total pulses for Keg Calibration
                "total_dispensed_pulses": 0
            }
            # Use the new tare_weight_kg key
    ...
    def get_system_settings(self):
        ...
        # --- FIX: Add EULA keys to the validation list ---
    ...
    def _calculate_volume_from_weight(self, total_weight_kg, empty_weight_kg, density=1.0):
    ...
    def _calculate_weight_from_volume(self, volume_liters, empty_weight_kg, density=1.0):
            ...
            try:
                with open(self.keg_library_file_path, 'r') as f: 
                    ...
                    for k in keg_list:
                        ...
                        # --- NEW: Migration & REPAIR for Pulse Counting ---
                        existing_liters = k.get('current_dispensed_liters', 0.0)
                        current_pulses = k.get('total_dispensed_pulses', 0)
                        
                        # Case 1: Key missing entirely (New migration)
                        if 'total_dispensed_pulses' not in k:
                            k['total_dispensed_pulses'] = int(existing_liters * DEFAULT_K_FACTOR)
                            library_was_modified = True
                            
                        # Case 2: Key exists but is 0 while Liters > 0 (Repair bad migration)
                        elif current_pulses == 0 and existing_liters > 0.01:
                            print(f"SettingsManager: Repairing pulses for {k.get('title')} (Liters: {existing_liters}, Pulses: {current_pulses})")
                            k['total_dispensed_pulses'] = int(existing_liters * DEFAULT_K_FACTOR)
                            library_was_modified = True
                        # --------------------------------------------------
                    ...
                    # --- END Key Migration Logic ---
                    
                    # FIX: If we updated the structure, save it to disk NOW.
    ...
    def _load_settings(self, force_defaults=False):
        ...
        # --- REMOVED: Clean up old/unused trial keys ---
        
        # [.. other unchanged settings loading/validation logic ..]
        ...
        # ... [remaining loading/validation logic] ...
        ...
        # --- FIXED SMTP PORT LOADING LOGIC ---
        port_val = notif_set.get('smtp_port', default_push_notification_settings_val['smtp_port'])
        
        # Ensure server is present (needed for the next step's exception handling to not crash if server is missing)
        ...
        try:
            ...
            if port_str.isdigit():
                ...
                # If it's an empty string or non-digit string, save as ""
                notif_set['smtp_port'] = "" 
        except Exception: 
            # If for some reason port_val is corrupted or missing, ensure it defaults to ""
            notif_set['smtp_port'] = ""
        # --- END FIXED SMTP PORT LOADING LOGIC ---
        
        # --- NEW: Status Request Settings Loading ---
        ...
        # Ensure IMAP/SMTP ports are handled correctly (stored as int/str)
        for key in ['imap_port', 'smtp_port']:
            ...
            try:
                ...
                status_req_set[key] = ""
        # --- END NEW: Status Request Settings Loading ---
