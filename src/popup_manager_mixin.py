...
class PopupManagerMixin:    
    ...
    def _save_keg_calibration(self, popup):
        try:
            ...
            # 2. Set the Tap to Offline (since the keg is kicked)
            self.settings_manager.save_sensor_keg_assignment(tap_idx, UNASSIGNED_KEG_ID)

            # 3. Force Logic Refresh (Recalculates with new K-Factor)
            if hasattr(self, 'sensor_logic') and self.sensor_logic:
                self.sensor_logic.force_recalculation()
            
            # 4. Force UI Refresh (Updates dropdowns to "Offline" and data display)
            ...
            messagebox.showinfo("Success", 
                                ...
                                "The tap has been set to 'Offline' because the keg is empty.", 
                                parent=popup)
            popup.destroy()
